...
class FullyAutomatedProductLinesKnowledgeManager(GraphKnowledgeBaseAPI):
    def __init__(self):
        self.database_name = os.environ.get(
            "KNOWLEDGE_DB_NAME", "evolutionKnowledgeBase"
        )
        self.database_user = os.environ.get("KNOWLEDGE_BASE_USERNAME", "neo4j")
        self.database_password = os.environ.get(
            "KNOWLEDGE_BASE_PASSWORD", "featureNeo4j"
        )
        self.database_bolt = os.environ.get(
            "KNOWLEDGE_BASE_DB_BOLT", "bolt://localhost:7688"
        )
