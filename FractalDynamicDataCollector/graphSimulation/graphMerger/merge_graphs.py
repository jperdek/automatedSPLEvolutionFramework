...
# for Windows only
from graphSimulation.graphMerger.merge_configuration import MergeConfiguration, ImageSettings, StrategyOnIntersection
...
class GraphMerger:
    ...
    def copy_node_content_according_schema(graph_json_node: dict, node_id: int,
                                           ...
                                           drawing: bool = True, merge_configuration: MergeConfiguration = None,
                                           instance_related_data: dict = None) -> dict:
        image_settings = None
        ...
        for schema_name, schema_type in schema_for_node.items():
            ...
            if connector_type_name == schema_name:
                ...
                if "." in schema_name:
                    ...
                    converted_checked_value = GraphMerger.convert_according_type(iterator_variable, schema_type)
                    node_content[schema_name] = converted_checked_value
                else:
                    node_content[schema_name] = None
        node_content.update({"id": node_id})
        RelatedDataManager.process_instance_related_data(instance_related_data, node_content, merge_configuration)
        return node_content
    ...
    @staticmethod
    def process_node(graph_json_node: dict, tree_graph_json: Optional[dict],
                     node_number_id: dict, connections: list[dict],
                     result_nodes: dict, graph_schema: dict, connector_list_name: str = "pointsTo",
                     connector_type_name: str = "fname",
                     drawing: bool = True,
                     merge_configuration: Optional[MergeConfiguration] = None, with_graphic_nodes: bool = False,
                     instance_related_data: dict = None) -> Optional[dict]:
        ...
        if tree_graph_json is not None:
            is_new_node = NodeVerifier.verify_actual_node_as_new_node(graph_json_node, tree_graph_json, graph_schema,
        ...
        # CREATING NEW NODE
        if is_new_node:
            node_content = GraphMerger.copy_node_content_according_schema(graph_json_node,
                                                                          node_number_id["id"], graph_schema[node_type],
                                                                          connector_list_name, connector_type_name,
                                                                          drawing, merge_configuration,
                                                                          instance_related_data=instance_related_data)
            if node_content[connector_type_name] not in GeometryDataToImage.FUNCTION_NAMES_TO_GENERATORS_MAPPING:
            ...
            if connector_list_name in graph_json_node.keys():
                for connected_node in graph_json_node[connector_list_name]:
                    neighbour_node = GraphMerger.process_node(connected_node, None,
                                                              node_number_id, connections, result_nodes, graph_schema,
                                                              connector_list_name, connector_type_name,
                                                              drawing, merge_configuration,
                                                              instance_related_data=instance_related_data)
                    ...
                    if neighbour_node is not None \
                            and (with_graphic_nodes or not neighbour_node[connector_type_name]
                                                           in GeometryDataToImage.FUNCTION_NAMES_TO_GENERATORS_MAPPING):
                        node_content[connector_list_name].append(neighbour_node)
                        connections.append(GraphMerger.create_connection(node_content, neighbour_node))
        ...
        # SEARCHING OLD ONES
            ...
            RelatedDataManager.process_instance_related_data(instance_related_data, tree_graph_json, merge_configuration)
                ...
                for connected_node in graph_json_node[connector_list_name]:
                    ...
                    for connected_node_tree in tree_graph_json[connector_list_name]:
                        if NodeVerifier.verify_actual_objects_according_schema(
                                connected_node, connected_node_tree, graph_schema, connector_type_name):

                            neighbour_node = GraphMerger.process_node(connected_node, connected_node_tree,
                                                                      node_number_id, connections, result_nodes,
                                                                      graph_schema,
                                                                      connector_list_name, connector_type_name,
                                                                      drawing, merge_configuration, with_graphic_nodes,
                                                                      instance_related_data=instance_related_data)
                            # if "id" not in connected_node.keys():
                            #    connected_node["id"] = neighbour_node["id"]
                            if neighbour_node is not None \
                                ...
                                if neighbour_node["id"] != connected_node_tree["id"]:
                                    tree_graph_json[connector_list_name].append(neighbour_node)
                                    connections.append(
                                        GraphMerger.create_connection(connected_node_tree, neighbour_node))
                            # ALREADY CONNECTED
                            found = True
                            break
                    ...
                    if not found:
                        ...
                        if neighbour_node is not None \
                            ...
                            if neighbour_node["id"] != tree_graph_json["id"]:
                                tree_graph_json[connector_list_name].append(neighbour_node)
                                connections.append(GraphMerger.create_connection(tree_graph_json, neighbour_node))

            MergeAggregations.mark_the_same_method(node_content)
        return node_content

    @staticmethod
    def get_nodes_and_connectors_from_graph_in_JSON(graph_json: dict, tree_graph_json: Optional[dict],
                                                    used_schema: dict, connections: list[dict],
                                                    node_number_id: dict, result_nodes: dict,
                                                    init_node: dict,
                                                    ...
                                                    instance_related_data: dict = None) -> (list[dict]):
            ...
            for index, graph_json_node in enumerate(nodes_to_iterate):
                if tree_graph_json:
                    ...
                    for connected_node_tree in tree_graph_json[connectors_list_name]:
                        if NodeVerifier.verify_actual_objects_according_schema(
                                graph_json_node, connected_node_tree,
                                used_schema[graph_json_node[connector_type_name]], connector_type_name):
                            neighbour_node = GraphMerger.process_node(graph_json_node, connected_node_tree,
                                                                      node_number_id, connections, result_nodes,
                                                                      used_schema,
                                                                      connectors_list_name, connector_type_name,
                                                                      drawing, merge_configuration, with_graphic_nodes,
                            ...
                            break
                    if not found:
                        neighbour_node = GraphMerger.process_node(graph_json_node, None,
                                                                  node_number_id, connections, result_nodes,
                                                                  used_schema,
                                                                  connectors_list_name, connector_type_name, drawing,
                                                                  merge_configuration, with_graphic_nodes,
                                                                  instance_related_data)
                        if "id" not in graph_json_node.keys():
                            graph_json_node["id"] = neighbour_node["id"]

                        if neighbour_node is not None \
                                and (with_graphic_nodes or not graph_json_node[connector_type_name] in
                                                               GeometryDataToImage.FUNCTION_NAMES_TO_GENERATORS_MAPPING):
                            tree_graph_json[connectors_list_name].append(neighbour_node)
                            connections.append(GraphMerger.create_connection(init_node, neighbour_node))
                else:
                    graph_json_node["id"] = node_number_id["id"]
                    node_number_id["id"] = node_number_id["id"] + 1
                    neighbour_node = GraphMerger.process_node(graph_json_node, None,
                                                              node_number_id, connections, result_nodes, used_schema,
                                                              connectors_list_name, connector_type_name, drawing,
                                                              merge_configuration, with_graphic_nodes,
                                                              instance_related_data)
        ...
        if tree_graph_json is None:
            graph_json[connectors_list_name] = graph_json[connectors_list_name] + newly_added_nodes
    ...
    @staticmethod
    def merge_dataset(dataset_directory_path: str,
                      final_location_path: str = "../../generated_dataset_vp_graph_data_merged",
                      graph_schema: dict = None, connectors_list_name: str = "pointsTo",
                      connector_type_name: str = "fname",
                      drawing: bool = True, with_graphic_nodes: bool = False,
                      merge_configuration: Optional[MergeConfiguration] = None,
                      instances_related_data: dict = None) -> None:
        ...
        absolute_connection_file_path = os.path.join(absolute_final_path, "connections.csv")
        connections = []
        node_number_id = {"id": 1}
        result_nodes = {}
        global_root = None
        ...
        for index, derivation_name in enumerate(os.listdir(absolute_dataset_path)):
            ...
            if instances_related_data is not None:
                instance_related_data_class = instances_related_data[derivation_name + ".png"]
            else:
            ...
            graph_root = json.loads(dynamic_fractal_analyzer.load_data_from_fractal(script_path, "initialGraphRoot"))
            GraphMerger.get_nodes_and_connectors_from_graph_in_JSON(
                graph_root, global_root, graph_schema, connections, node_number_id, result_nodes, init_node,
                connectors_list_name, connector_type_name, drawing, with_graphic_nodes,
                merge_configuration=merge_configuration, instance_related_data=instance_related_data_class)
        ...
        for node_type_name, result_nodes_type in result_nodes.items():
            node_type_absolute_path = os.path.join(absolute_final_path, node_type_name + ".csv")
            if len(result_nodes_type) > 0:  # create file only if will contain any data/ entities
                for i, result_type in enumerate(result_nodes_type):
                    if "pointsTo" in result_type.keys():
                        del result_nodes_type[i]["pointsTo"]
                GraphMerger.create_csv_from_results(node_type_absolute_path,
                                                    result_nodes_type, list(all_keys_dict.values()))
        GraphMerger.create_csv_from_results(absolute_connection_file_path, connections, list(connections[0].keys()))

    @staticmethod
    def load_instance_related_data(path_to_object_csv: str, delimiter: str = ";") -> dict:
        whole_data = {}
...
if __name__ == "__main__":
    ...
    GraphMerger.merge_dataset("E://aspects/src/derived2", with_graphic_nodes=False,
                              merge_configuration=merge_configuration, instances_related_data=instance_related_data)
