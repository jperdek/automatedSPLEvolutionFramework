...
class NodeVerifier:
    ...
    @staticmethod
    def verify_actual_objects_according_schema(graph_json_node: dict,
                                               graph_tree_json_node: dict, schema_for_node: dict,
                                               connector_type_name: str = "fname"):
        if graph_json_node[connector_type_name] != graph_tree_json_node[connector_type_name]:
            return False

        for schema_name, schema_type in schema_for_node.items():
            if schema_name == connector_type_name or schema_name in NodeVerifier.DISAGREE_ON_VERIFICATION:
                return False
            if schema_name == "id" or schema_name in NodeVerifier.SKIP_VERIFICATION:
                ...
                if "." in schema_name:
                    ...
                    if skip1:
                        ...
                        try:
                            converted_checked_value1 = NodeVerifier.convert_according_type(iterator_variable1, schema_type)
                        except:
                            converted_checked_value1 = NodeVerifier.convert_according_type(graph_tree_json_node[schema_name], schema_type)
                    ...
                    if skip:
                        ...
                        try:
                            converted_checked_value2 = NodeVerifier.convert_according_type(iterator_variable2, schema_type)
                        except:
                            converted_checked_value2 = NodeVerifier.convert_according_type(graph_tree_json_node[schema_name],
                                                                                           schema_type)
                    if converted_checked_value1 != converted_checked_value2:
                        return False
                else:
    ...
    @staticmethod
    def verify_neighbour_graphic_nodes(graph_json_node: dict,
                                       tree_graph_json: Optional[dict], graph_schema: dict,
            ...
            for connected_node in graph_json_node[connector_list_name]:
                ...
                if graphic_object_name in \
                    ...
                    for connected_node_tree in tree_graph_json[connector_list_name]:
                        if tree_graph_json[connector_list_name] in \
                                GeometryDataToImage.FUNCTION_NAMES_TO_GENERATORS_MAPPING.keys():
                            if not NodeVerifier.verify_actual_objects_according_schema(
                                    connected_node, connected_node_tree, graphic_object_schema):
                                return False
        return True

    @staticmethod
    def verify_actual_node_as_new_node(graph_json_node: dict,
                                       tree_graph_json: Optional[dict],
                                       graph_schema: dict, connector_list_name: str = "pointsTo",
                                       connector_type_name: str = "fname") -> bool:
        if tree_graph_json is None: # or graph_json_node[connector_type_name] != tree_graph_json[connector_type_name]:
            return True
        schema_for_node = graph_schema[graph_json_node[connector_type_name]]
        if not NodeVerifier.verify_actual_objects_according_schema(graph_json_node, tree_graph_json, schema_for_node):
            return True
        if not NodeVerifier.verify_neighbour_graphic_nodes(graph_json_node, tree_graph_json,
                                                           graph_schema, connector_list_name, connector_type_name):
            return True
        return False
