...
class SchemaConverter:
    ...
    @staticmethod
    def copy_node_content_according_schema(inserted_graph_node: dict, schema_for_node: dict,
                                           remove_dot: bool = True) -> dict:
        node_content = {}
...
class GraphConnector:
    ...
    def convert_schema_dict_to_neo_parameters(object_schema: dict) -> str:
        ...
        for variable_name in object_schema.keys():
            ...
            neo4j_parameters = neo4j_parameters + " " + variable_name + ": $" + variable_name + ","
    ...
    def insert_objects(self, path_to_object_csv: str, object_schema: dict,
                       object_name: str = "Obj", delimiter: str = "$"):
        ...
        with self.driver.session() as session:
            with open(path_to_object_csv, "r", encoding="utf-8") as file:
                ...
                for new_object_record in new_objects:
                    new_object_record_schema = SchemaConverter.copy_node_content_according_schema(
                        new_object_record, object_schema, remove_dot=True)
                    session.run("MERGE (a: " + object_name + " "+ neo4j_parameters + ");", new_object_record_schema)

    def insert_connections(self, path_to_connections_csv: str, delimiter: str = "$") -> None:
        with self.driver.session() as session:
            with open(path_to_connections_csv, "r", encoding="utf-8") as file:
                ...
                for connection_record in connections:
                    session.run("MATCH (a {id: " + connection_record["from"] + "} ), (b {id: " +
                                connection_record["to"] + " }) MERGE (a)-[c: CONNECTS]->(b);")
...
def insert_merged_graph(url: str, user: str, password: str) -> None:
    ...
    connection.insert_objects("../../generated_dataset_vp_graph_data_merged/function drawWCurve.csv",
                              FractalGraphScheme.create_default_scheme_for_draw_WCurve(), "DrawWCurve")
    connection.insert_objects("../../generated_dataset_vp_graph_data_merged/function WCurve.csv",
                              FractalGraphScheme.create_default_scheme_for_wcurve(), "WCurve")
    ...
    connection.insert_connections("../../generated_dataset_vp_graph_data_merged/connections.csv")
    connection.close()
...
def insert_small_graph(url: str, user: str, password: str) -> None:
    connection = GraphConnector(url, user, password)
    connection.clear_database()
    connection.insert_objects("../../generated_dataset_vp_graph_data/1/function drawWCurve.csv",
                              FractalGraphScheme.create_default_scheme_for_draw_WCurve(), "DrawWCurve")
    connection.insert_objects("../../generated_dataset_vp_graph_data/1/function WCurve.csv",
                              FractalGraphScheme.create_default_scheme_for_wcurve(), "WCurve")
    connection.insert_connections("../../generated_dataset_vp_graph_data/1/connections.csv")
    connection.close()
