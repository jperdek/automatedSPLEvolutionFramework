...
class DynamicFractalAnalyzer:
    ...
    def load_fractal(self, fractal_script_path: str) -> str:
        ...
        branch_data_script = fractal_content + "; function getData() { return JSON.stringify(globalResult); } getData();"
        return self.__get_branches(branch_data_script)

    def load_data_from_fractal(self, fractal_script_path: str, variable_name: str) -> str:
        with open(fractal_script_path, "r", encoding="utf-8") as file:
            fractal_content = file.read()
        fractal_content = self.__remove_context_content(fractal_content)
        branch_data_script = fractal_content + "; function getData() { return JSON.stringify(" \
                             + variable_name + " ); } getData();"
        return self.__get_branches(branch_data_script)
