...
class RecursionCollector:
    ...
    def __construct_type_search_expression(used_type: str) -> str:
        ...
        for type, type_search_value in RecursionCollector.TYPES_DEF.items():
            if used_type == "all" or not used_type or type in used_type:
                variable_search_formula = (
                    variable_search_formula + type_search_value + "|"
                )
    ...
    @staticmethod
    def extract_associated_variable_from_pair_in_key(
        program_identifier: str,
        variability_point_data: dict,
        variable_name: str,
        variable_type: str = "all",
        allowed_other_types: str = "all",
    ) -> (list[dict], list[str]):
        variable_search_expression = (
            "_"
            + variable_name
            + RecursionCollector.__construct_type_search_expression(variable_type)
        )
        analyzed_third_dependency = (
            r"([^_]+)_"
        ...
        for variable_combination in variability_point_data.keys():
            ...
            search_expression = (
                variable_search_expression + "__" + analyzed_third_dependency
            )
            variable_value, other_variable_name, other_variable_value = regexp.search(
                search_expression, dependent_variables_part
            ).groups()
    ...
    @staticmethod
    def __generate_variable_template(
        program_identifier: str, variable_name: str, variable_value: Union[int, str]
    ) -> dict:
        variable_template = {"Name": program_identifier, variable_name: variable_value}
        return variable_template

    @staticmethod
    def extract_variable_values_from_combination_of_key(
        program_identifier: str,
        variable_name: str,
        collected_data: dict,
        variable_type: str = "all",
        allowed_other_types: str = "all",
        number_related: int = 1,
    ) -> (list[dict], list[str]):
        variable_search_expression = (
            "_"
            + variable_name
        ...
        for variable_combination in collected_data.keys():
            dependencies_regexp = regexp.search(
                r"" + search_expression, variable_combination
            )
            ...
            for other_variable_name, other_variable_value in zip(
                variables_data[1::2], variables_data[2::2]
            ):
                variable_template[other_variable_name] = other_variable_value
                if other_variable_name not in result_names:
                    result_names.append(other_variable_name)
    ...
    def evaluate_dataset(
        path_to_dataset: str,
        analyzed_variable_name: str = "iteration",
        number_related_ones: int = 1,  # 8
        variability_point_name: str = "1-1-1",
    ) -> (list[dict], list[str]):
        ...
        for file_name in os.listdir(path_to_dataset):
            ...
            program_variability_point_data = (
                RecursionCollector.__load_variability_point_data(path_to_program_json)
            )

            (
                result,
                column_names,
            ) = RecursionCollector.extract_variable_values_from_combination_of_key(
                file_name,
                analyzed_variable_name,
                program_variability_point_data[variability_point_name],
                "all",
                "all",
                number_related_ones,
            )
...
if __name__ == "__main__":
    ...
    evaluator.create_csv_from_results(
        "harvested_data_point.csv", results_all, result_column_names
    )
