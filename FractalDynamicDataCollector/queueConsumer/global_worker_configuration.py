...
class DataRepresentationsClient:
    @staticmethod
    def get_destination_dataset_path(
        evolution_iteration: str, project_unique_id: str
    ) -> str:
        destination_dataset_path = (
            os.getenv("PATH_TO_DATASET_DIRECTORY", "../../temp")
            + "/evolution"
            + str(evolution_iteration)
            + "/"
            + project_unique_id
        )
    ...
    @staticmethod
    def copy_whole_project(
        evolved_spl_path: str,
        resulting_spl_project_path: str,
        ...
        try:
            shutil.copytree(
                evolved_spl_path,
                resulting_spl_project_path + "/code",
                dirs_exist_ok=True,
            )
    ...
    def get_and_save_screenshot(
        evolved_spl_index_path: str,
        resulting_spl_project_path: str,
        logger: Optional[logging.Logger] = None,
    ) -> Optional[str]:
        service_url = (
            DataRepresentationsClient.get_server_path()
            + "/api/screenshoter/screenshoter?url=file:///"
            + evolved_spl_index_path
        )
        response = requests.get(DataRepresentationsClient.__clean_url(service_url))
    ...
    def get_and_save_graph_data(
        ...
        service_url = (
            DataRepresentationsClient.get_server_path()
            + "/api/graph-extraction/extractObject?max_depth=70&url=file://"
            + evolved_spl_index_path
        )
        ...
        if response.status_code == 200:
            ...
            if logger:
                logger.debug(
                    "Error :-> Saving single SPL instance graph :-> Request: "
                    + service_url
                    + " failed with status code: "
                    + str(response.status_code)
                )
    ...
    def get_and_save_svg_data(
        ...
        service_url = (
            DataRepresentationsClient.get_server_path()
            + "/api/svg_creator/create_svg?time_to_wait=10&generated_svg_id=mySvg&draw_line_in_op=drawLine&replacement_function=function(context, x1, y1, x2, y2, thickness)&url=file://"
            + evolved_spl_index_path
        )
        response = requests.get(service_url)
    ...
    def save_knowledge_after_iteration(
        ...
        evolved_script_path: str,
        code_path: str,
        screenshot_path: str,
        vector_path: str,
        json_graph_path: str,
        variation_point_data_location: str,
        ...
        request_data = {
            ...
            "evolved_script_path": evolved_script_path,
            "variation_point_data_location": variation_point_data_location,
    ...
    def create_all_representations(
        ...
        destination_spl_project_path = (
            DataRepresentationsClient.get_destination_dataset_path(
                evolution_iteration, project_id
            )
        )
        evolved_spl_path = evolved_spl_path.strip().replace("&#39;", "").strip("'")
        destination_spl_project_path = (
            destination_spl_project_path.strip().replace("&#39;", "").strip("'")
        )
    ...
    def test(logger: Optional[logging.Logger] = None):
        ...
        DataRepresentationsClient.create_all_representations(
            evolved_spl_path,
            evolved_spl_script_path,
            evolution_iteration,
            project_id,
            logger,
        )
