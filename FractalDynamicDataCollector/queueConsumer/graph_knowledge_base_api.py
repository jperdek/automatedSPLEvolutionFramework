...
class GraphKnowledgeBaseAPI:
    ...
    def create_db_for_network(self, db_name: Optional[str] = None) -> None:
        if not db_name:
    ...
    def initialize_co_occurrence_indexes(self, db_name: Optional[str] = None):
        if not db_name:
            db_name = self.database_name
        with self.driver.session() as session:
    ...
    @staticmethod
    def _init_co_occurrence_indexes(tx, db_name: Optional[str] = None):
