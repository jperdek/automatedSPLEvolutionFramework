...
from semantic_base.knowledge_graph.neosemantics_graph import (
    NeosemanticsKnowledgeGraphApi,
)
...
default_knowledge_graph_api = Blueprint(
    "default_knowledge_graph_api", __name__, template_folder="templates"
)
...
def register_new_evolution_iteration():
    ...
    if not evolved_product_line_id:
        return "No evolved software product line id provided.", 500, {
    ...
    evolved_script_id = evolved_script_path[evolved_script_path.rfind("/") + 1 :].split(
        "."
    )[0]

    base_header = request_data.get(
        "base_header",
        "@base <https://jakubperdek-26e24f.gitlab.io/fully-automated-spls-schema.ttl> .",
    )

    if "fully_automated_product_lines_knowledge_manager" not in g:
        g.fully_automated_product_lines_knowledge_manager = (
            FullyAutomatedProductLinesKnowledgeManager()
        )
...
def register_new_evolution():
    ...
    if not initial_product_line_id:
        return "No initial product line id provided.", 500, {
            "content-type": "text/plain"
        }
...
def add_triples():
    ...
    g.fully_automated_product_lines_knowledge_manager.process_data_transaction_using_commands_block(
        knowledge_ttl_call
    )
