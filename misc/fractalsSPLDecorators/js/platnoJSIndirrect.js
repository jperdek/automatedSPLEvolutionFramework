...
*/
@wholeEntity({"outsideGallery": "true"})
...
*/
@wholeInitlialization({"outsideGallery": "true"})
var gameInfo = {
	circles: [],
	thinLineThickness: 1,
	lines: []
...
if ( true ) {
	...
	*/
	...
	function drawWCurve( iteration, moveRatioIteration, centerX, centerY, wcurve, context, direction, @variableArgument({"outsideGallery": "true"}) inheritedOperation){
		...
		@variableDeclaration({"newFunctionParamGallery": "true"})
        let newA;
		...
		if(iteration > 0){
			...
			if(iteration == 1){
				...
				//~{}
                @wholeInitialization({"outsideGallery": "true"}) 
                let cond = true;
                @wholeBlock({"outsideGallery": "true"}) function skipFunction() { return "value" };
				if(direction == wcurve.direction.LEFT_DOWN){
					;
					//~{}
