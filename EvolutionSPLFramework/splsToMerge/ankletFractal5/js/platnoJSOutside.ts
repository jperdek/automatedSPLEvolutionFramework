...
/*
	...
	*/
window.export enum Orientation {
	TOP = 0,
	LEFT = 1,
	RIGHT = 2,
	BOTTOM = 3,
...
window.export function drawAnkletModMain(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, circleRadius: number, 
			iterationsNumber: number, lineLength: number, thickness: number, squareSideLength: number, ankletMod: AnkletMod): void {
        ankletMod.drawAnklet(iterationsNumber, Math.round(canvas.width/2.0), Math.round(canvas.height / 2.0),  ankletMod, context, Orientation.ALL, false);
}