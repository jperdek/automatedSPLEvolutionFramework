...
window.export class AnkletInfo {
	...
	*/
	...
	/*
		...
		@param {Context} context - context from Canvas to perform paint operations
	*/
	createBase(centerX: number, centerY: number, ankletInfo: AnkletInfo, context: CanvasRenderingContext2D): void {
		const centerXMinusHypo = centerX - ankletInfo.hypotenuseSquareHalf;
		const centerYMinusHypo = centerY - ankletInfo.hypotenuseSquareHalf;
		const centerXPlusHypo = centerX + ankletInfo.hypotenuseSquareHalf;
		const centerYPlusHypo = centerY + ankletInfo.hypotenuseSquareHalf;

		drawLine(context, centerX, centerYMinusHypo, centerXPlusHypo, centerY, ankletInfo.thickness);
		drawLine(context, centerXPlusHypo, centerY, centerX, centerYPlusHypo, ankletInfo.thickness);
		drawLine(context, centerX, centerYPlusHypo, centerXMinusHypo, centerY, ankletInfo.thickness);
		drawLine(context, centerXMinusHypo, centerY, centerX, centerYMinusHypo, ankletInfo.thickness);
	}
...
window.export function drawAnkletMain(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, circleRadius: number, 
			iterationsNumber: number, lineLength: number, thickness: number, squareSideLength: number, ankletInfo: AnkletInfo): void {
	ankletInfo.drawAnklet(iterationsNumber, lineLength, Math.round(canvas.width/2.0), Math.round(canvas.height / 2.0),  ankletInfo, context);
}