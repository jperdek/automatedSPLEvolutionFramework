...
from keras.utils import to_categorical, img_to_array, plot_model  # Converts a class vector (integers) to binary class matrix.
...
model.compile(loss="categorical_crossentropy", optimizer=opt,
              metrics=["accuracy"])
plot_model(model, to_file='modelNativeShrink.png', show_shapes=True, show_layer_names=True)
...
if USE_VALIDATION_SET:
    ...
    # A metric function is similar to a loss function, except that the results
    # from evaluating a metric are not used when training the model.
    history = model.fit(data, trainY, batch_size=BS, epochs=EPOCHS, validation_split=0.2)
