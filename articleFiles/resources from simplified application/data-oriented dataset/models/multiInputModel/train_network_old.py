...
class LeNetExtended:
    ...
    @staticmethod
    def build_input_parts(associated_variable_names: list[str], max_value_array_size: int, max_variables=5) -> (list[layers], list[layers]):
        all_new_end_layers = []
        all_new_start_layers = []
        for associated_variable_name in associated_variable_names:
            new_layer_input = Input(batch_shape=(None, max_value_array_size),
...
input_layers_start, input_layers_end = LeNetExtended.build_input_parts(["centerX", "centerY"],
                                                                       max_value_array_size=MAX_VALUE_ARRAY_SIZE)
...
if USE_VALIDATION_SET:
    ...
    }, trainY, batch_size=BS, epochs=EPOCHS)
